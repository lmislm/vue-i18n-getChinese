...
import { StringLiteral, TemplateLiteral, DirectiveLiteral } from '@babel/types';
...
function extractStringFromBabelAST(ast: any, stringList: StringASTType[], filter: { (str: string): boolean }) {
  if ('ImportDeclaration' === ast.type) {
    ...
    return;
  } else if (ast.callee && ast.callee.object && ast.callee.object.name === 'console') {
    // 忽略console开头的表达式打印信息
    return;
    ...
    if (ast.expressions && ast.expressions.length && ast.quasis.length > 1) {
      if (
        ast.quasis.some((_ast: TemplateLiteral) => {
          if (_ast.value) {
...
export default {
  ...
  extractStringFromJS(
    jsCode: string,
    { filter = () => true, startNo = 0 }: ExtractStringOptions = {},
  ): ExtractStringResult {
    // 收集的导出字符串
    ...
    stringASTList.forEach((item: any) => {
      if (item.type == 'String') {
        ...
        if (item.first) {
          ...
          let originalCode = jsCode.substring(
            item.item.start - 1 + codeLengthChange,
            item.item.end + 2 + codeLengthChange,
          );
          jsCode =
            jsCode.substring(0, item.item.start - 1 + codeLengthChange) +
            replaceString +
            jsCode.substring(item.item.end + 2 + codeLengthChange);
          ...
          let originalCode = jsCode.substring(
            item.item.start - 1 + codeLengthChange,
            item.item.end + 1 + codeLengthChange,
          );
          ...
          jsCode =
            jsCode.substring(0, item.item.start - 1 + codeLengthChange) +
            replaceString +
            jsCode.substring(item.item.end + 1 + codeLengthChange);

          // 修正replaceCode和originalCode
          let replaceCode = jsCode.substring(
            cacheTemplateMap[index].start,
            item.item.start - 1 + codeLengthChange + replaceString.length,
          );
