              ...
              if (item.name.match(/^[:@]|v-/)) {
                ...
                try {
                  ...
                  if (result.extractString.length > 0) {
                    ...
                    jsCode =
                      jsCode.substr(0, item.start + changeLength) +
                      prefixCode +
                      '=' +
                      item.quotationMarks +
                      result.result +
                      item.quotationMarks +
                      jsCode.substr(item.end + changeLength);
        ...
        if (topTagName === 'template') {
          ...
          while (last) {
              ...
              if (endIndex !== -1) {
                ...
                try {
                  ...
                  if (result.extractString.length > 0) {
                    ...
                    extractStrings = extractStrings.concat(_extractStrings);
                    jsCode =
                      jsCode.substr(0, _start + changeLength) +
                      '{{' +
                      result.result +
                      '}}' +
                      jsCode.substr(_start + code.length + 4 + changeLength);
                    changeLength += result.result.length - code.length;
            ...
            if (filter(code)) {
              ...
              jsCode =
                jsCode.substr(0, _start + changeLength) +
                replaceCode +
                jsCode.substr(_start + code.length + changeLength);
              changeLength += replaceCode.length - extractString.originalCode.length;
          ...
          try {
            ...
            if (result.extractString.length > 0) {
              ...
              extractStrings = extractStrings.concat(_extractStrings);
              jsCode = jsCode.substr(0, start + changeLength) + result.result + jsCode.substr(end + changeLength);
              changeLength += result.result.length - code.length;
            }
          } catch (e) {
            console.warn(code, '\n', e);
